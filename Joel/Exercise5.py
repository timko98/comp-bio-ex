...
def gaussian_likelihood(mu, sigma2, x):
    ...
    if isinstance(mu, float):
        return 1 / ((2 * np.pi * sigma2) ** (x.size / 2)) * np.exp((-x.size / (2 * sigma2)) * ((np.mean(x) - mu) ** 2 + np.var(x)))
    lik = np.zeros(mu.size)
    for i in range(mu.size):
        lik[i] = 1 / ((2 * np.pi * sigma2) ** (x.size / 2)) * np.exp((-x.size / (2 * sigma2)) * ((np.mean(x) - mu[i]) ** 2 + np.var(x)))
    return lik
