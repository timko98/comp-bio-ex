...
def Z(lambda1, lambda2):
    ...
    0.16666666666666666
    '''
    z = 1 / ((lambda1 * lambda2) * (lambda1 + lambda2))
    return (z)
...
def lambdas(mean_a, mean_b):
    ...
    3.0488597087860572]
    '''
    lambda1_1 = 3 / (2 * mean_a - mean_b + sqrt(mean_a ** 2 - mean_a * mean_b + mean_b ** 2))
    lambda1_2 = 3 / (2 * mean_a - mean_b - sqrt(mean_a ** 2 - mean_a * mean_b + mean_b ** 2))
    lambda2_1 = 3 / (2 * mean_b - mean_a + sqrt(mean_a ** 2 - mean_a * mean_b + mean_b ** 2))
    lambda2_2 = 3 / (2 * mean_b - mean_a - sqrt(mean_a ** 2 - mean_a * mean_b + mean_b ** 2))
    result = [lambda1_1, lambda1_2, lambda2_1, lambda2_2]
    return (result)
...
def get_means_xyz(mean_a, mean_b):
    ...
    '''
    lambda_values = lambdas(mean_a, mean_b)
    result = []
    result.append(1 / (lambda_values[0] + lambda_values[2]))
    result.append(1 / lambda_values[0])
    result.append(1 / (lambda_values[2]))
    return (result)
...
def get_covariance(mean_a, mean_b):
    ...
    mean_xyz = get_means_xyz(mean_a, mean_b)
    cov_mat[1,0] = mean_xyz[0]**2
    cov_mat[0,1] = mean_xyz[0]**2
    cov_mat[0,0] = mean_xyz[0]**2 + mean_xyz[1]**2
    cov_mat[1,1] = mean_xyz[0]**2 + mean_xyz[2]**2
    return (cov_mat)
